...
import { Microscope, Truck, Lock, FileText } from "lucide-react";

export default function LabGrid() {
  return (
    <section className="relative py-24 px-4 md:px-8 max-w-7xl mx-auto z-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[300px]">
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="md:col-span-2 relative rounded-3xl border border-[var(--glass-border)] bg-[var(--bg-page)]/50 backdrop-blur-sm overflow-hidden group hover:border-[var(--color-brand-primary)]/50 transition-colors duration-500"
        >
        ...
        <motion.div 
           ...
           className="relative rounded-3xl border border-[var(--glass-border)] bg-[var(--bg-page)]/50 backdrop-blur-sm p-8 flex flex-col justify-between hover:border-[var(--color-brand-secondary)]/50 transition-colors duration-500"
        >
           <div>
             <div className="w-12 h-12 rounded-xl bg-[var(--color-brand-secondary)]/10 flex items-center justify-center mb-4">
           ...
           <div className="w-full h-1 bg-[var(--glass-border)] rounded-full overflow-hidden mt-4">
        ...
        <motion.div 
           ...
           className="relative rounded-3xl border border-[var(--glass-border)] bg-[var(--bg-page)]/50 backdrop-blur-sm p-8 flex flex-col justify-between hover:border-[var(--text-main)]/30 transition-colors duration-500"
        >
           <div>
           ...
           <div className="flex items-center gap-2 text-[var(--color-brand-secondary)] text-xs font-mono">
              <span className="w-2 h-2 rounded-full bg-current animate-pulse" />
              SECURE_CONNECTION
           </div>
        </motion.div>

        {/* Card 4: Documents (Wide) */}
        <motion.div 
           initial={{ opacity: 0, y: 20 }}
           whileInView={{ opacity: 1, y: 0 }}
           viewport={{ once: true }}
           transition={{ delay: 0.3 }}
           className="md:col-span-2 relative rounded-3xl border border-[var(--glass-border)] bg-[var(--bg-page)]/50 backdrop-blur-sm p-8 flex items-center overflow-hidden hover:border-[var(--color-brand-primary)]/50 transition-colors duration-500"
        >
             <div className="relative z-10 max-w-md">
                 ...
                 <button className="text-xs font-bold uppercase tracking-widest text-[var(--text-main)] border border-[var(--glass-border)] px-6 py-3 rounded-lg hover:bg-[var(--text-main)] hover:text-[var(--bg-page)] transition-all flex items-center gap-2">
                    View Compliance Docs <FileText className="w-4 h-4" />
                 </button>
             </div>
             
             {/* Decoración Molecular Abstracta */}
             <div className="absolute right-[-50px] top-[-50px] opacity-10 rotate-12">
                <div className="w-64 h-64 border-[20px] border-[var(--text-muted)] rounded-full border-dashed animate-[spin_60s_linear_infinite]" />
             </div>
        </motion.div>
