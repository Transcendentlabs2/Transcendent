...
export default function ProductForm({ onClose, initialData }: ProductFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!imageUrl) {
        Swal.fire({ icon: 'warning', title: 'Missing Image', background: '#1a1a1a', color: '#fff' });
        setLoading(false);
        return;
    }
    ...
    if (result.success) {
        Swal.fire({
            ...
            background: '#1a1a1a', color: '#fff', confirmButtonColor: '#0ea5e9',
            timer: 1500, showConfirmButton: false
        });
        onClose();
    } else {
        Swal.fire({ icon: 'error', title: 'Error', text: result.message, background: '#1a1a1a', color: '#fff' });
    }
  };
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto bg-[var(--bg-page)] border border-[var(--glass-border)] rounded-2xl shadow-2xl flex flex-col max-h-[90vh] md:max-h-[85vh]">
      ...
      <div className="flex items-center justify-between p-5 border-b border-[var(--glass-border)] bg-[var(--bg-page)] sticky top-0 z-20 rounded-t-2xl">
        ...
        <button onClick={onClose} className="p-2 hover:bg-red-500/10 hover:text-red-400 rounded-lg transition-colors">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Imagen Cloudinary (Visualización Mejorada) */}
            <div className="space-y-2">
                <label className="text-xs font-bold text-[var(--text-muted)] uppercase">Visual Documentation</label>
                <div className="border-2 border-dashed border-[var(--glass-border)] rounded-xl p-4 flex justify-center bg-[#0a0a0a]">
                    {imageUrl ? (
                        <div className="relative w-full h-48 md:w-64 md:h-64">
                            {/* AQUÍ ESTÁ EL TRUCO: object-contain para que no se recorte */}
                        ...
                        <CldUploadWidget uploadPreset={process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET} onSuccess={(result: any) => setImageUrl(result.info.secure_url)}>
                            {({ open }) => (
                                <button type="button" onClick={() => open()} className="flex flex-col items-center gap-2 text-[var(--text-muted)] hover:text-[var(--color-brand-primary)] py-8">
      ...
      <div className="flex justify-end gap-3 p-5 border-t border-[var(--glass-border)] bg-[var(--bg-page)] rounded-b-2xl sticky bottom-0 z-20">
            ...
            <button 
                onClick={(e) => {
                    // Hack para disparar el submit del form desde fuera
                    const form = e.currentTarget.closest('.relative')?.querySelector('form');
                    form?.requestSubmit();
                }}
                disabled={loading} 
                className="bg-[var(--text-main)] text-[var(--bg-page)] px-6 py-2.5 rounded-xl font-bold text-sm hover:scale-105 transition-all flex items-center gap-2 shadow-lg shadow-white/10"
            >
      ...
      <style jsx global>{`
        .input-scientific {
            width: 100%;
            background-color: var(--bg-page); /* Asegúrate de que esta variable exista o usa #000 */
