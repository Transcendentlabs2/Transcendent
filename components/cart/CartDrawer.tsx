        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-[var(--glass-border)] shrink-0">
              ...
              <button onClick={toggleCart} className="p-2 hover:bg-[var(--glass-border)] rounded-full transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 min-h-0 overscroll-y-contain custom-scrollbar">
              {items.length === 0 ? (
                ...
                items.map((item) => {
                  ...
                  return (
                    <motion.div layout key={item.id} className={`flex gap-3 bg-[var(--glass-bg)] border p-3 rounded-xl relative group overflow-hidden transition-colors ${isItemOOS ? "border-red-500/30 bg-red-500/5" : "border-[var(--glass-border)]"}`}>
                      ...
                      <div className="flex-1 flex flex-col justify-between min-w-0">
                        ...
                        <div className="flex items-end justify-between mt-2">
                          <div className={`flex items-center gap-3 bg-[var(--bg-page)] rounded-lg border border-[var(--glass-border)] px-2 py-1 ${isItemOOS ? "opacity-30 pointer-events-none" : ""}`}>
                            <button onClick={() => updateQuantity(item.id, Math.max(1, item.quantity - 1))} className="text-[var(--text-muted)] hover:text-[var(--text-main)]"><Minus className="w-3 h-3" /></button>
                            <span className="text-xs font-mono font-bold w-4 text-center">{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.id, Math.min(item.stock || 99, item.quantity + 1))} className="text-[var(--text-muted)] hover:text-[var(--text-main)]"><Plus className="w-3 h-3" /></button>
                          </div>
                          <span className={`font-mono font-bold ${isItemOOS ? "text-[var(--text-muted)] line-through" : "text-[var(--color-brand-primary)]"}`}>${(item.price * item.quantity).toFixed(2)}</span>
                        </div>
                      </div>
                    </motion.div>
                  );
